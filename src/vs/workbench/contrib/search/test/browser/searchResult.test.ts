...
suite('SearchResult', () => {
	...
	test('replace should remove the file match', function () {
		const orinPromise = Promise.resolve(null);
		instantiationService.stub(IReplaceService, 'replace', orinPromise);
		const testObject = aSearchResult();
		addToSearchResult(testObject, [
			aRawMatch('/1',
				new TextSearchMatch('preview 1', lineOneRange))]);
		...
		return orinPromise.then(() => assert.ok(testObject.isEmpty()));
	});
	...
	test('replaceAll should remove all file matches', function () {
		const orinPromise = Promise.resolve(null);
		instantiationService.stubPromise(IReplaceService, 'replace', orinPromise);
		const testObject = aSearchResult();
		addToSearchResult(testObject, [
			aRawMatch('/1',
				new TextSearchMatch('preview 1', lineOneRange)),
