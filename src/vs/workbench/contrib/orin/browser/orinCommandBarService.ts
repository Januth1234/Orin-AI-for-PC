...
import { mountOrinCommandBar } from './react/out/orin-editor-widgets-tsx/index.js'
...
import { ORIN_ACCEPT_DIFF_ACTION_ID, ORIN_REJECT_DIFF_ACTION_ID, ORIN_GOTO_NEXT_DIFF_ACTION_ID, ORIN_GOTO_PREV_DIFF_ACTION_ID, ORIN_GOTO_NEXT_URI_ACTION_ID, ORIN_GOTO_PREV_URI_ACTION_ID, ORIN_ACCEPT_FILE_ACTION_ID, ORIN_REJECT_FILE_ACTION_ID, ORIN_ACCEPT_ALL_DIFFS_ACTION_ID, ORIN_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface IOrinCommandBarService {
...
export const IOrinCommandBarService = createDecorator<IOrinCommandBarService>('OrinCommandBarService');
...
export class OrinCommandBarService extends Disposable implements IOrinCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'orin.OrinCommandBarService'
...
registerSingleton(IOrinCommandBarService, OrinCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type OrinCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountOrinCommandBar(root, accessor, { uri, editor } satisfies OrinCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies OrinCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ORIN_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('orinAcceptDiffAction', 'Orin: Accept Diff'),
			keybinding: {
				primary: KeyMod.CtrlCmd | KeyMod.Alt | KeyMod.Shift | KeyCode.Enter,
				mac: { primary: KeyMod.WinCtrl | KeyMod.Alt | KeyCode.Enter },
				weight: KeybindingWeight.OrinExtension,
			}
		});
	}
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const commandBarService = accessor.get(IOrinCommandBarService);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ORIN_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('orinRejectDiffAction', 'Orin: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ORIN_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('orinGoToNextDiffAction', 'Orin: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ORIN_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('orinGoToPrevDiffAction', 'Orin: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ORIN_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('orinGoToNextUriAction', 'Orin: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ORIN_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('orinGoToPrevUriAction', 'Orin: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ORIN_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('orinAcceptFileAction', 'Orin: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ORIN_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('orinRejectFileAction', 'Orin: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ORIN_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('orinAcceptAllDiffsAction', 'Orin: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ORIN_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('orinRejectAllDiffsAction', 'Orin: Reject All Diffs in All Files'),
			keybinding: {
