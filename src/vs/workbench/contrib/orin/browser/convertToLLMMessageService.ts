...
import { ChatMode, FeatureName, ModelSelection, ProviderName } from '../common/orinSettingsTypes.js';
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	private _getOrinRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let orinRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.voidrules')
				const { model } = this.orinModelService.getModel(uri)
				if (!model) continue
				orinRules += model.getValue(EndOfLinePreference.LF) + '\n\n';
			}
			return orinRules.trim();
		}
		catch (e) {
			return ''
		}
	...
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.orinSettingsService.state.globalSettings.aiInstructions;
		const orinRulesFileContent = this._getOrinRulesFileContents();

		const ans: string[] = []
		if (globalAIInstructions) ans.push(globalAIInstructions)
		if (orinRulesFileContent) ans.push(orinRulesFileContent)
		return ans.join('\n\n')
	}
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.orinSettingsService.state
		...
		const modelSelectionOptions = this.orinSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const { disableSystemMessage } = this.orinSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat)
		const systemMessage = disableSystemMessage ? '' : fullSystemMessage;

		const modelSelectionOptions = this.orinSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]
