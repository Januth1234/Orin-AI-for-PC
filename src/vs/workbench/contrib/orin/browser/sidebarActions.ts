...
import { ORIN_VIEW_CONTAINER_ID, ORIN_VIEW_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { ORIN_TOGGLE_SETTINGS_ACTION_ID } from './orinSettingsPane.js';
...
const ORIN_OPEN_SIDEBAR_ACTION_ID = 'orin.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: ORIN_OPEN_SIDEBAR_ACTION_ID, title: localize2('orinOpenSidebar', 'Orin: Open Sidebar'), f1: true });
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		const viewsService = accessor.get(IViewsService)
		const chatThreadsService = accessor.get(IChatThreadService)
		viewsService.openViewContainer(ORIN_VIEW_CONTAINER_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ORIN_CTRL_L_ACTION_ID,
			f1: true,
			title: localize2('orinCmdL', 'Orin: Add Selection to Chat'),
			keybinding: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(ORIN_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(ORIN_OPEN_SIDEBAR_ACTION_ID)
		}
...
const ORIN_CMD_SHIFT_L_ACTION_ID = 'orin.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ORIN_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
			...
			icon: { id: 'add' },
			menu: [{ id: MenuId.ViewTitle, group: 'navigation', when: ContextKeyExpr.equals('view', ORIN_VIEW_ID), }],
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		commandService.executeCommand(ORIN_CMD_SHIFT_L_ACTION_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'orin.settingsAction',
			title: `Orin's Settings`,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(ORIN_TOGGLE_SETTINGS_ACTION_ID)
	}
})
