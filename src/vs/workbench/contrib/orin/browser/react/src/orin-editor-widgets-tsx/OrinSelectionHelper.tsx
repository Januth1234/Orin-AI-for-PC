...
import '../styles.css'
import { ORIN_CTRL_K_ACTION_ID, ORIN_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { Circle, MoreVertical } from 'lucide-react';
import { useEffect, useState } from 'react';

import { OrinSelectionHelperProps } from '../../../../../../contrib/orin/browser/orinSelectionHelperWidget.js';
import { ORIN_OPEN_SETTINGS_ACTION_ID } from '../../../orinSettingsPane.js';


export const OrinSelectionHelperMain = (props: OrinSelectionHelperProps) => {
	...
	>
		<OrinSelectionHelper {...props} />
	</div>
}



const OrinSelectionHelper = ({ rerenderKey }: OrinSelectionHelperProps) => {
	...
	const ctrlLKeybind = keybindingService.lookupKeybinding(ORIN_CTRL_L_ACTION_ID)
	const ctrlKKeybind = keybindingService.lookupKeybinding(ORIN_CTRL_K_ACTION_ID)

	const dividerHTML = <div className='w-[0.5px] bg-orin-border-3'></div>
	...
	useEffect(() => {
		const disposable = commandService.onWillExecuteCommand(e => {
			if (e.commandId === ORIN_CTRL_L_ACTION_ID || e.commandId === ORIN_CTRL_K_ACTION_ID) {
	...
	const defaultHTML = <>
		{ctrlLKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(ORIN_CTRL_L_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
		...
		{ctrlKKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(ORIN_CTRL_K_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
	...
	const moreOptionsHTML = <>
		<div
			...
			'
			onClick={() => {
				commandService.executeCommand(ORIN_OPEN_SETTINGS_ACTION_ID);
				setClickState('clickedOption');
			}}
		>
			Disable Suggestions?
	...
	return <div className='
		...
		border border-orin-border-3 bg-orin-bg-2
