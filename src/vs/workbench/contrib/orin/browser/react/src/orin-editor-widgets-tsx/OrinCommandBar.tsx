...
import { OrinCommandBarProps } from '../../../orinCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	ORIN_GOTO_NEXT_DIFF_ACTION_ID,
	ORIN_GOTO_PREV_DIFF_ACTION_ID,
	ORIN_GOTO_NEXT_URI_ACTION_ID,
	ORIN_GOTO_PREV_URI_ACTION_ID,
	ORIN_ACCEPT_FILE_ACTION_ID,
	ORIN_REJECT_FILE_ACTION_ID,
	ORIN_ACCEPT_ALL_DIFFS_ACTION_ID,
	ORIN_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const OrinCommandBarMain = ({ uri, editor }: OrinCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@orin-scope ${isDark ? 'dark' : ''}`}
	>
		<OrinCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const OrinCommandBar = ({ uri, editor }: OrinCommandBarProps) => {
	...
	const orinModelService = accessor.get('IOrinModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(ORIN_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(ORIN_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(ORIN_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(ORIN_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(ORIN_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(ORIN_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(ORIN_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(ORIN_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-orin-bg-2 shadow-md border border-orin-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-orin-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-orin-bg-2 rounded shadow-md border border-orin-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-orin-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-orin-bg-2 rounded shadow-md border border-orin-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-orin-border-2 [&>*:last-child]:border-r-0">
