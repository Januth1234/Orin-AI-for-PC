...
export const OrinInputBox2 = forwardRef<HTMLTextAreaElement, InputBox2Props>(function X({ initValue, placeholder, multiline, enableAtToMention, fnsRef, className, onKeyDown, onFocus, onBlur, onChangeText }, ref) {
	...
	return <>
		<textarea
			...
			className={`w-full resize-none max-h-[500px] overflow-y-auto text-orin-fg-1 placeholder:text-orin-fg-3 ${className}`}
			style={{
		...
		{isMenuOpen && (
			<div
				...
				className="z-[100] border-orin-border-3 bg-orin-bg-2-alt border rounded shadow-lg flex flex-col overflow-hidden"
				style={{
					position: strategy,
					top: y ?? 0,
					left: x ?? 0,
			...
			>
				{/* Breadcrumbs Header */}
				{isBreadcrumbsShowing && <div className="px-2 py-1 text-orin-fg-1 bg-orin-bg-2-alt border-b border-orin-border-3 sticky top-0 bg-orin-bg-1 z-10 select-none pointer-events-none">
					{optionText ?
				...
				<div className='max-h-[400px] w-full max-w-full overflow-y-auto overflow-x-auto'>
					<div className="w-max min-w-full flex flex-col gap-0 text-nowrap flex-nowrap">
						{options.length === 0 ?
							<div className="text-orin-fg-3 px-3 py-0.5">No results found</div>
							: options.map((o, oIdx) => {
								...
								return (
									...
									<div
										...
										className={`
											...
											${oIdx === optionIdx ? 'bg-blue-500 text-white/80' : 'bg-orin-bg-2-alt text-orin-fg-1'}
										`}
...
export const OrinSimpleInputBox = ({ value, onChangeValue, placeholder, className, disabled, passwordBlur, compact, ...inputProps }: {
	value: string;
	...
	return (
		<input
			...
			placeholder={placeholder}
			disabled={disabled}
			className={`w-full resize-none bg-orin-bg-1 text-orin-fg-1 placeholder:text-orin-fg-3 border border-orin-border-2 focus:border-orin-border-1
...
export const OrinInputBox = ({ onChangeText, onCreateInstance, inputBoxRef, placeholder, isPasswordField, multiline }: {
	onChangeText: (value: string) => void;
	...
	return <WidgetComponent
		className='
			bg-orin-bg-1
			@@orin-force-child-placeholder-orin-fg-1
		'
		ctor={InputBox}
				...
				<div className="relative w-full">
					...
					<div
						className={`relative ${size === 'xxs' ? 'h-0.5' :
							size === 'xs' ? 'h-1' :
								size === 'sm' ? 'h-1.5' :
									size === 'sm+' ? 'h-2' : 'h-2.5'
							} bg-orin-bg-2 rounded-full cursor-pointer`}
						onClick={handleTrackClick}
					>
					...
					<div
						className={`absolute top-1/2 transform -translate-x-1/2 -translate-y-1/2
							...
							bg-orin-fg-1 rounded-full shadow-md ${disabled ? 'cursor-not-allowed' : 'cursor-grab active:cursor-grabbing'}
							border border-orin-fg-1`}
...
export const OrinCheckBox = ({ label, value, onClick, className }: { label: string, value: boolean, onClick: (checked: boolean) => void, className?: string }) => {
...
export const OrinCustomDropdownBox = <T extends NonNullable<any>>({
	options,
	selectedOption,
	onChangeOption,
	...
	return (
		<div className={`inline-block relative ${className}`}>
			...
			{isOpen && (
				<div
					ref={refs.setFloating}
					className="z-[100] bg-orin-bg-1 border-orin-border-3 border rounded shadow-lg"
					style={{
						position: strategy,
						top: y ?? 0,
						left: x ?? 0,
...
export const _OrinSelectBox = <T,>({ onChangeSelection, onCreateInstance, selectBoxRef, options, className }: {
	...
	return <WidgetComponent
		className={`
			...
			@@[&_select]:!orin-text-orin-fg-3
			@@[&_select]:!orin-text-xs
			!text-orin-fg-3
			${className ?? ''}
		`}
		ctor={SelectBox}
...
export const BlockCode = ({ initValue, language, maxHeight, showScrollbars }: BlockCodeProps) => {
	...
	return <div ref={divRef} className='relative z-0 px-2 py-1 bg-orin-bg-3'>
		<WidgetComponent
...
export const OrinButtonBgDarken = ({ children, disabled, onClick, className }: { children: React.ReactNode; disabled?: boolean; onClick: () => void; className?: string }) => {
...
// export const OrinScrollableElt = ({ options, children }: { options: ScrollableElementCreationOptions, children: React.ReactNode }) => {
...
// export const OrinSelectBox = <T,>({ onChangeSelection, initVal, selectBoxRef, options }: {
// 	initVal: T;
// 	selectBoxRef: React.MutableRefObject<SelectBox | null>;
// 	options: readonly { text: string, value: T }[];
// 	onChangeSelection: (value: T) => void;
...
// export const OrinCheckBox = ({ onChangeChecked, initVal, label, checkboxRef, }: {
...
const SingleDiffEditor = ({ block, lang }: { block: ExtractedSearchReplaceBlock, lang: string | undefined }) => {
	...
	return (
		<div className="w-full bg-orin-bg-3 @@bg-editor-style-override" ref={divRef} />
	);
};
 ...
 */
export const OrinDiffEditor = ({ uri, searchReplaceBlocks, language }: { uri?: any, searchReplaceBlocks: string, language?: string }) => {
	const accessor = useAccessor();
	const languageService = accessor.get('ILanguageService');
	...
	if (blocks.length === 0) {
		return <div className="w-full p-4 text-orin-fg-4 text-sm">No changes found</div>;
	}
		...
		<div className="w-full flex flex-col gap-2">
			{blocks.map((block, index) => (
				<div key={index} className="w-full">
					{blocks.length > 1 && (
						<div className="text-orin-fg-4 text-xs mb-1 px-1">
