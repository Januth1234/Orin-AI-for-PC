...
import { FeatureName, ModelSelection, ModelSelectionOptions } from '../common/orinSettingsTypes.js';
...
import { OrinFileSnapshot } from '../common/editCodeServiceTypes.js';
...
export const IChatThreadService = createDecorator<IChatThreadService>('orinChatThreadService');
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	// should probably re-use code from orin/src/vs/base/common/marshalling.ts instead. but this is simple enough
	...
	private _setState(state: Partial<ThreadsState>, doNotRefreshMountInfo?: boolean) {
		...
		if (streamState?.isRunning === undefined && !streamState?.error) {
			...
			// if awaiting user but stream state doesn't indicate it (happens if restart Orin)
			if (lastMessage && lastMessage.role === 'tool' && lastMessage.type === 'tool_request')
				this._setStreamState(threadId, { isRunning: 'awaiting_user', })

			// if running now but stream state doesn't indicate it (happens if restart Orin), cancel that last tool
	...
	private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
		const orinFileSnapshot = checkpointMessage.orinFileSnapshotOfURI ? checkpointMessage.orinFileSnapshotOfURI[fsPath] ?? null : null
		if (!opts.includeUserModifiedChanges) { return { orinFileSnapshot, } }

		const userModifiedOrinFileSnapshot = fsPath in checkpointMessage.userModifications.orinFileSnapshotOfURI ? checkpointMessage.userModifications.orinFileSnapshotOfURI[fsPath] ?? null : null
		return { orinFileSnapshot: userModifiedOrinFileSnapshot ?? orinFileSnapshot, }
	}
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const orinFileSnapshotOfURI: { [fsPath: string]: OrinFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			...
			if (!res) continue
			const { orinFileSnapshot: oldOrinFileSnapshot } = res
			...
			const orinFileSnapshot = this._editCodeService.getOrinFileSnapshot(URI.file(fsPath))
			if (oldOrinFileSnapshot === orinFileSnapshot) continue
			orinFileSnapshotOfURI[fsPath] = orinFileSnapshot
		}
		...
		return { orinFileSnapshotOfURI }
	}


	private _addUserCheckpoint({ threadId }: { threadId: string }) {
		const { orinFileSnapshotOfURI } = this._computeNewCheckpointInfo({ threadId }) ?? {}
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'user_edit',
			orinFileSnapshotOfURI: orinFileSnapshotOfURI ?? {},
			userModifications: { orinFileSnapshotOfURI: {}, },
		})
	}
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		...
		const diffAreasSnapshot = this._editCodeService.getOrinFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
			orinFileSnapshotOfURI: { [uri.fsPath]: diffAreasSnapshot },
			userModifications: { orinFileSnapshotOfURI: {} },
		})
	}
	...
	private _getCheckpointsBetween({ threadId, loIdx, hiIdx }: { threadId: string, loIdx: number, hiIdx: number }) {
		...
		for (let i = loIdx; i <= hiIdx; i += 1) {
			...
			for (const fsPath in message.orinFileSnapshotOfURI) { // do not include userModified.beforeStrOfURI here, jumping should not include those changes
	...
	private _addUserModificationsToCurrCheckpoint({ threadId }: { threadId: string }) {
		...
		this._editMessageInThread(threadId, checkpointIdx, {
			...checkpoint,
			userModifications: { orinFileSnapshotOfURI: orinFileSnapshotOfURI ?? {}, },
		})
	}
		...
		if (toIdx < fromIdx) {
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					if (!res) continue
					const { orinFileSnapshot } = res
					if (!orinFileSnapshot) continue
					this._editCodeService.restoreOrinFileSnapshot(URI.file(fsPath), orinFileSnapshot)
					break
				}
			}
		}
