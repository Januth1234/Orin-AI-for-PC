...
class FilePromptActionService extends Action2 {
	private static readonly ORIN_COPY_FILE_PROMPT_ID = 'orin.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.ORIN_COPY_FILE_PROMPT_ID,
			title: localize2('orinCopyPrompt', 'Orin: Copy Prompt'),
			menu: [{
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await orinModelService.getModelSafe(uri)).model?.getLanguageId() || '',
