...
export class OrinMainUpdateService extends Disposable implements IOrinUpdateService {
	...
	async check(explicit: boolean): Promise<OrinCheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart Orin to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<OrinCheckUpdateRespose> {
		try {
			const response = await fetch('https://api.github.com/repos/orinai/binaries/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of Orin is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'Orin is up-to-date!'
					}
				}
				else {
