...
export interface IOrinModelService {
	readonly _serviceBrand: undefined;
	initializeModel(uri: URI): Promise<void>;
	getModel(uri: URI): OrinModelType;
	getModelFromFsPath(fsPath: string): OrinModelType;
	getModelSafe(uri: URI): Promise<OrinModelType>;
	saveModel(uri: URI): Promise<void>;

}

export const IOrinModelService = createDecorator<IOrinModelService>('orinOrinModelService');

class OrinModelService extends Disposable implements IOrinModelService {
	_serviceBrand: undefined;
	static readonly ID = 'orinOrinModelService';
	...
	getModelSafe = async (uri: URI): Promise<OrinModelType> => {
...
registerSingleton(IOrinModelService, OrinModelService, InstantiationType.Eager);
