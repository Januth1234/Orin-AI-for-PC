...
import { ORIN_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, OrinStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './orinSettingsTypes.js';
...
// type RealOrinSettings = Exclude<keyof OrinSettingsState, '_modelOptions'>
// type EventProp<T extends RealOrinSettings = RealOrinSettings> = T extends 'globalSettings' ? [T, keyof OrinSettingsState[T]] : T | 'all'


export interface IOrinSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: OrinSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: OrinSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: OrinStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, OrinStatefulModelInfo> = {}
...
const _stateWithMergedDefaultModels = (state: OrinSettingsState): OrinSettingsState => {
...
const _validatedModelState = (state: Omit<OrinSettingsState, '_modelOptions'>): OrinSettingsState => {
...
export const IOrinSettingsService = createDecorator<IOrinSettingsService>('OrinSettingsService');
class OrinSettingsService extends Disposable implements IOrinSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: OrinSettingsState;
	...
	dangerousSetState = async (newState: OrinSettingsState) => {
	...
	private async _readState(): Promise<OrinSettingsState> {
		const encryptedState = this._storageService.get(ORIN_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(ORIN_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: OrinSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: OrinStatefulModelInfo[] = [
...
registerSingleton(IOrinSettingsService, OrinSettingsService, InstantiationType.Eager);
