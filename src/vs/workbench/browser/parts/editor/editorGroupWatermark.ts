...
/* eslint-disable */ // Orin
import { ORIN_CTRL_K_ACTION_ID, ORIN_CTRL_L_ACTION_ID } from '../../../contrib/orin/browser/actionIDs.js';
...
export class EditorGroupWatermark extends Disposable {
	...
	private render(): void {
		...
		this.clear();
		const orinIconBox = append(this.shortcuts, $('.watermark-box'));
		...
		const update = async () => {
			...
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				orinIconBox.appendChild(buttonContainer);
				...
				if (recentlyOpened.length !== 0) {
					...
					orinIconBox.append(
						...recentlyOpened.map((w, i) => {
							...
							const linkSpan = $('span');
							linkSpan.classList.add('orin-link')
				...
				// show them Orin keybindings
				const keys = this.keybindingService.lookupKeybinding(ORIN_CTRL_L_ACTION_ID);
				const dl = append(orinIconBox, $('dl'));
				...
				const keys2 = this.keybindingService.lookupKeybinding(ORIN_CTRL_K_ACTION_ID);
				const dl2 = append(orinIconBox, $('dl'));
